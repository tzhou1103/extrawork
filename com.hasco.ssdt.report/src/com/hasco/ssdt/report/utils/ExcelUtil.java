...
{	
	...
	{
		Dispatch range = Dispatch.invoke(sheet, "Range", Dispatch.Get, new Object[] { cellName }, new int[1]).toDispatch();
		Variant left = Dispatch.get(range, "Left");
		Variant top = Dispatch.get(range, "Top");
		Dispatch.call(range, "Select"); //在工作表中，定位需要插入图片的具体位置
		Dispatch select = Dispatch.call(sheet, "Shapes").toDispatch();
		Dispatch.call(select, "AddPicture", new Object[] { imagePath, false, true, left, top, -1, -1 }).toDispatch();
	}
	...
	/**
	 * 按照页数，将当前sheet页里的模板表格复制出多页
	 * 
	 * @param sheet
	 * @param startRowNo	开始行
	 * @param endRowNo		结束行
	 * @param pageNum 		页数
	 * @param interval 		页与页的表头之间间隔的行数
	 */
	public static void copySheetRows(Dispatch sheet, int startRowNo, int endRowNo, int pageNum, int interval)
	{
		Dispatch rowArea = Dispatch.invoke(sheet, "Rows", Dispatch.Get, new Object[] { startRowNo + ":" + endRowNo }, new int[1]).toDispatch();
		Dispatch.call(rowArea, "Select");
		Dispatch.call(rowArea, "Copy");
		
		for (int i = 1; i < pageNum; i++)
		{
			int targetRow = interval * i + 1;
			Dispatch targetRowArea = Dispatch.invoke(sheet, "Rows", Dispatch.Get, new Object[] { targetRow + ":" + targetRow }, new int[1]).toDispatch();
			Dispatch.call(targetRowArea, "Select");
