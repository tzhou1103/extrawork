...
public class NewMultipleWeldingProcedureSpecificationJob extends Job 
{
	private TCComponentBOMLine baplantBOPBOMLine;
	...
	public NewMultipleWeldingProcedureSpecificationJob(String name, TCComponentBOMLine baplantBOPBOMLine, int languageType, String chexingVariant) 
	{
		super(name);
		this.baplantBOPBOMLine = baplantBOPBOMLine;
	...
	protected IStatus run(IProgressMonitor progressMonitor) 
	{
		...
		try 
		{
			...
			TcUtil.willExpand(new TCComponentBOMLine[] {this.baplantBOPBOMLine});
			
			Vector<TCComponentBOMLine> bastationBOMLineVector = new Vector<TCComponentBOMLine>();
			
			traverseBOMLine(this.baplantBOPBOMLine, bastationBOMLineVector);
			
			for (int i = 0; i < bastationBOMLineVector.size(); i++) 
			{
				TCComponentBOMLine bastationBOMLine = bastationBOMLineVector.get(i);
				
				writeExcel(bastationBOMLine, workingDirFile);
			}
	...
	private void writeExcel(TCComponentBOMLine baStationBOMLine, File workingDirFile) throws Exception
	{
		String workingDir = workingDirFile.getAbsolutePath();
		
		File templateFile = null;
		boolean updateStatus = false;
		TCComponentItem weldingProcedureItem = findWeldingProcedureSpecification(baStationBOMLine);
		...
		String bastationID = baStationBOMLine.getProperty("bl_item_item_id");
		
		TCComponentBOMLine stationBOMLine = null;
		int workerCount = 0;
		Vector<TCComponentBOMLine> opBOMLineVector = new Vector<TCComponentBOMLine>();
		AIFComponentContext[] aaifContext = baStationBOMLine.getChildren();
...
//				}
		...
		aaifContext = baStationBOMLine.getItemRevision().getChildren();
		for (int i = 0; i < aaifContext.length; i++) 
		{
		...
		try 
		{
			...
			if (stationBOMLine != null)
			{
				...
				String postAddress = getPostAddress(baStationBOMLine);
		...
		if (saveFile != null)
		{
			if (weldingProcedureItem == null)
			{
				File resultFile = new File(saveFile.getParent(), "WeldingProcedureSpecification_" + System.currentTimeMillis() + TcUtil.getSuffix(saveFile.getName()));
...
//					weldingProcedureItem.getLatestItemRevision().setProperty("s4_AT_DocumentType", "焊装_工艺卡");
				...
				baStationBOMLine.getItemRevision().add("IMAN_reference", weldingProcedureItem);
	...
	private void traverseBOMLine(TCComponentBOMLine paramBOMLine, Vector<TCComponentBOMLine> bastationBOMLineVector) throws Exception
	{
		...
		for (int i = 0; i < aaifContext.length; i++) 
		{
			...
			if (bomLine.getItem().isTypeOf("S4_IT_BAStation"))
			{
				TCComponentItem wPSpecification = findWeldingProcedureSpecification(bomLine);
				if (wPSpecification != null)
				{
					TCComponent[] status = wPSpecification.getLatestItemRevision().getRelatedComponents("release_status_list");
					if ((status == null) || (status.length <= 0))
					{
						bastationBOMLineVector.add(bomLine);
					}
				}else
				{
					bastationBOMLineVector.add(bomLine);
			...
			{
				traverseBOMLine(bomLine, bastationBOMLineVector);
			}
		}
	}
