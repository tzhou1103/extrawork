...
/**
 * Implementation of the ExceptionHandler. For ConnectionExceptions (server temporarily down .etc) prompts the user to
 * retry the last request. For other exceptions convert to a RunTime exception.
 */
public class AppXExceptionHandler implements ExceptionHandler {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.teamcenter.soa.client.ExceptionHandler#handleException(com.teamcenter
	 * .schemas.soa._2006_03.exceptions.InternalServerException)
	 */
	public void handleException(InternalServerException ise) {
		LoggerDefault.logWarn("");
		LoggerDefault.logWarn("*****");
		LoggerDefault.logWarn("Exception caught in com.teamcenter.clientx.AppXExceptionHandler.handleException(InternalServerException).");
		...
		// "The user has opted not to retry the last request");
		// } catch (IOException e) {
		// System.err
		// .println("Failed to read user response.\nA RuntimeException will be thrown.");
	...
	public void handleException(CanceledOperationException coe) {
		LoggerDefault.logWarn("");
		LoggerDefault.logWarn("*****");
		LoggerDefault.logWarn("Exception caught in com.teamcenter.clientx.AppXExceptionHandler.handleException(CanceledOperationException).");
