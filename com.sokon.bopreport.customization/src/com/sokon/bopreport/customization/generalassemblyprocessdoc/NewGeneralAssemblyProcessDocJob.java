...
public class NewGeneralAssemblyProcessDocJob extends Job 
{

	private TCComponentBOMLine gaOpBOMLine;
	private int languageType;
	private Registry appReg;
	private String chexingVariant;
	private boolean flag = false;
	public NewGeneralAssemblyProcessDocJob(String name, TCComponentBOMLine gaOpBOMLine, int languageType, String chexingVariant) 
	{
		super(name);
		this.gaOpBOMLine = gaOpBOMLine;
	...
	protected IStatus run(IProgressMonitor progressMonitor) 
	{
		...
		try 
		{
			...
			TCComponentItem assemblyProcessCard = findAssemblyProcessDoc(this.gaOpBOMLine);
			...
			AIFComponentContext[] aaifContext = this.gaOpBOMLine.parent().getChildren();
			...
			aaifContext = this.gaOpBOMLine.getChildren();
			for (int i = 0; i < aaifContext.length; i++) 
			{
				TCComponentBOMLine bomLine = (TCComponentBOMLine) aaifContext[i].getComponent();
			...
			aaifContext = this.gaOpBOMLine.getItemRevision().getChildren();
			for (int i = 0; i < aaifContext.length; i++) 
			{
...
//					File[] imageFiles = ds.getFiles("ThumbnailImage", workingDir);
			...
			try 
			{
				...
				TcUtil.writeData(sheet, "A7", this.gaOpBOMLine.getProperty("bl_item_item_id"));
				
				String gxName = "";
				if (this.languageType == 0)
				{
					gxName = this.gaOpBOMLine.getProperty("bl_rev_s4_CAT_ChineseName");
					
				}else if (this.languageType == 1)
				{
					gxName = this.gaOpBOMLine.getProperty("bl_rev_object_name");
				}else if (this.languageType == 2)
				{
					gxName = this.gaOpBOMLine.getProperty("bl_rev_s4_CAT_ChineseName") + "\n" + this.gaOpBOMLine.getProperty("bl_rev_object_name");
				}
				
				TcUtil.writeData(sheet, "E7", gxName);
				
				TcUtil.writeData(sheet, "AB7", this.gaOpBOMLine.getProperty("bl_rev_item_revision_id"));
				...
				if (stationBOMLine != null)
				{
					...
					String procResArea = "";
					TCComponentBOMLine processResourceBOMLine = (TCComponentBOMLine)this.gaOpBOMLine.getRelatedComponent("Mfg0processResource");
				...
				TcUtil.writeData(sheet, "AV7", this.gaOpBOMLine.getProperty("Mfg0allocated_time"));
				...
				for (int j = 0; j < jzsVector.size(); j++) 
				{
					...
					String remark = jzsRemarkMap.get(jzsBOMLine.getItem()).toString();
				...
				for (int j = 0; j < avxVector.size(); j++) 
				{
					...
					if (partRev != null)
						...
						xinghao = avxBOMLine.getItemRevision().getProperty("s4_AT_AuxiliarySpec");
...
//					String formulaStr = getFormulaStr(formula);
			...
			if (saveFile != null)
			{
				if (assemblyProcessCard == null)
				{
					File resultFile = new File(saveFile.getParent(), "GeneralAssemblyProcessDocuments" + TcUtil.getSuffix(saveFile.getName()));
...
//						assemblyProcessCard.getLatestItemRevision().setProperty("s4_AT_DocumentType", "总装_工艺卡");
					...
					this.gaOpBOMLine.getItemRevision().add("IMAN_reference", assemblyProcessCard);
