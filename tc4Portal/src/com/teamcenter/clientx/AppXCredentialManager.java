...
/**
 * The CredentialManager is used by the Teamcenter Services framework to get the user's credentials when challenged by
 * the server. This can occur after a period of inactivity and the server has timed-out the user's session, at which
 * time the client application will need to re-authenticate. The framework will call one of the getCredentials methods
 * (depending on circumstances) and will send the SessionService.login service request. Upon successful completion of
 * the login service request. The last service request (one that caused the challenge) will be resent.
 * 
 * The framework will also call the setUserPassword setGroupRole methods when ever these credentials change, thus
 * allowing this implementation of the CredentialManager to cache these values so prompting of the user is not required
...
public class AppXCredentialManager implements CredentialManager {
	...
	/**
	 * Return the type of credentials this implementation provides, standard (user/password) or Single-Sign-On. In this
	 * case Standard credentials are returned.
	 * 
	...
	/**
	 * Prompt's the user for credentials. This method will only be called by the framework when a login attempt has
	 * failed.
	 * 
	...
	public String[] getCredentials(InvalidCredentialsException e) throws CanceledOperationException {
		LoggerDefault.logWarn(e.getMessage());
		return promptForCredentials();
	}

	/**
	 * Return the cached credentials. This method will be called when a service request is sent without a valid session
	...
	/**
	 * Cache the group and role This is called after the SessionService.setSessionGroupMember service operation is
	 * called.
	 * 
	...
	/**
	 * Cache the User and Password This is called after the SessionService.login service operation is called.
	 * 
	...
	public String[] promptForCredentials() throws CanceledOperationException {
		try {
			...
			LoggerDefault.logError(message);
