...
public class DSExport 
{
		...
		case PDF:
			...
			for (int j = 0; j < files.length; j++) {
				for (int i = 0; i < currentTransferPath.size(); i++) {
					nxPartToPath.put(files[j].getAbsolutePath() + "@#@"
							+ (index++), currentTransferPath.get(i));
				}
			}
			break;
	...
	public static void export(String datasetType, TCComponentDataset dataSetObject, String exportPath) throws TCException 
	{
		...
		case PDF:
			dataSetObject.getFiles("PDF_Reference", exportPath);
			break;

		case Image:
			dataSetObject.getFiles("Image", exportPath);
			break;

		case MSWord:
			dataSetObject.getFiles("word", exportPath);
			break;
		...
		case MSExcel:
			dataSetObject.getFiles("excel", exportPath);
			break;
		...
		case H5Winrar:
			dataSetObject.getFiles("H5Winrar", exportPath);
			break;

		case Zip:
			dataSetObject.getFiles("ZIPFILE", exportPath);
			break;
