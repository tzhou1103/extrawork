...
public class DataManagement
{
    ...
    public void createReviseAndDelete()
    {
        try
        {
			...
			reviseItems(allRevIds, itemRevs, new String[itemRevs.length]);
     ...
     */
    @SuppressWarnings("unchecked")
	public Map<String, ReviseOutput> reviseItems(Map<BigInteger,RevisionIds> revisionIds, ItemRevision[] itemRevs, String[] revIDPrefixArray) throws ServiceException, NotLoadedException
    {
        // Get the service stub
        DataManagementService dmService = DataManagementService.getService(AppXSession.getConnection());
         ...
         for (int i = 0; i < itemRevs.length; i++)
        {
            ...
            reviseInfo[i].description      = "Revise by webservice";
			dmService.getProperties(new ModelObject[] { itemRevs[i] }, new String[] { "object_name" });
            reviseInfo[i].name             = itemRevs[i].get_object_name();
            reviseInfo[i].newRevId         = revIDPrefixArray[i] + rev.newRevId;
        ...
        return revised.reviseOutputMap;
    }
